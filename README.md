...
1. 审阅首版英文文案与项目取舍。
2. 补充 Google Scholar / ORCID（如公开）；首页头像已获授权并加入首屏。
3. 明确哪些 manuscript-in-preparation 项目可以公开；未确认内容当前不会构建。

首版未引入 React、Tailwind、CMS、数据库或服务端适配器；内容使用 Astro 内容集合和 Markdown，保持静态输出。2026-08-10 起，站点新增独立的 Poetry 内容集合与 `/blog/poetry/` 静态栏目；中文原文为必填，英文译文可后续补充；有译文时按英文在前、中文在后的顺序展示。未引入完整博客、RSS、搜索或评论系统。
...
About、Interests、Education 和 Contact 合并进 Home；Interests 与 Education 使用响应式双栏并直接归入个人陈述区域，首页个人陈述简短概括代表工作，不另设 Selected work 或 Technical toolkit 板块。CV 只作为内容来源，不创建下载文件或独立公开路由。Poetry 已因首篇真实内容而公开；其他 Writing/Posts 类型仍只保留内容模型，不创建空页面或主导航入口。
